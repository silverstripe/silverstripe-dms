...
if ($config->get(DMSDocument_versions::class, 'enable_versions')) {
    ...
    $config->update(DMSDocument_versions::class, 'db', $config->get(DMSDocument::class, 'db'));
}
...
if (strpos($config->get(DMS::class, 'folder_name'), 'assets/') === 0) {
    ...
    $config->update(Filesystem::class, 'sync_blacklisted_patterns', array("/^" . $folderName . "$/i",));
}
